...
-(void)loadFinished
{
    ...
    if (self.mDevicePeripheral != nil)
    {
        ...
        {
            [self updateCurrentSignalStrength:[mManager queryDistance:self.mDevicePeripheral]];
            NSLog(@"[CachedBLEDevice] [loadFinished] alert state from blemanager : %d", [mManager getIsNotifyRemote:self.mDevicePeripheral]);
            self.mAlertState = [mManager getIsNotifyRemote:self.mDevicePeripheral];
        }
    }
}
...
-(void)updateDeviceConfiguration:(int)which changedValue:(int)value
{
    ...
    if (changed == YES)
    {
        [mManager updatePxpSetting:mDeviceIdentifier alertEnabler:_mAlertEnabled range:_mRangeAlertEnabled rangeType:_mRangeType alertDistance:_mRangeValue disconnectAlertEnabler:_mDisconnectEnabled];
