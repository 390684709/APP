...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    float level = (_yValueMax-_yValueMin) /8.0;
    CGFloat chartCavanHeight = self.frame.size.height - UULabelHeight*3;
    CGFloat levelHeight = chartCavanHeight /8.0;
    ...
    for (int i=4; i<9; i++) {
        if (i==4||i==8) {
            if ([_ShowHorizonLine[i] integerValue]>0) {
                ...
                shapeLayer.strokeColor = [[[UIColor blackColor] colorWithAlphaComponent:0.6] CGColor];
    ...
    for (int i=6; i<7; i++) {
        if ([_ShowHorizonLine[i] integerValue]>0) {
            ...
            CAShapeLayer *shapeLayer = [CAShapeLayer layer];
            UIBezierPath *path = [UIBezierPath bezierPath];
            [path moveToPoint:CGPointMake(UUYLabelwidth,UULabelHeight+i*levelHeight-10)];
            [path addLineToPoint:CGPointMake(self.frame.size.width,UULabelHeight+i*levelHeight-10)];
            [path closePath];
            shapeLayer.path = path.CGPath;
            shapeLayer.strokeColor = [[[UIColor blackColor] colorWithAlphaComponent:0.6] CGColor];
            shapeLayer.fillColor = [[UIColor whiteColor] CGColor];
