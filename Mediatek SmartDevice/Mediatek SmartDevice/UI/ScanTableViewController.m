...
//        UINavigationController* controllerWithHeart = [self.storyboard instantiateViewControllerWithIdentifier:@"MainViewControllerWithHeart"];
//        [self presentViewController:controllerWithHeart animated:YES completion:nil];
    }
    else
    {
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isGetedHardwareVersion:) name:@"toGetHardwareVersion" object:nil];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isGetMtkBLEOn:) name:@"toGetMtkBLEOn" object:nil];
...
-(void)viewDidDisappear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"toGetHardwareVersion" object:nil];
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"toGetMtkBLEOn" object:nil];
...
//            if([[BackgroundManager sharedInstance] connectDevice:pp])
//            {
//                [self toConnectBLE];
//            }
        }
...
//        if ([mManager.foundPeripherals count] != 0)
        if ([foundedDevices count] != 0)
        {
            ...
            NSLog(@"[ScanTableViewController] [cellForRowAtIndexPath] scanned device name : %@", deviceTitle);
...
//        device.mRangeAlertEnabled = true;binge update
        device.mRangeAlertEnabled = false;
...
-(void)toScanBLE
{
    if (!self.isConnected) {
        ...
        [NSTimer  scheduledTimerWithTimeInterval:0.5f  target:self  selector:@selector(toScanBLE) userInfo:nil  repeats:NO];
    }else {
...
-(void)toGetHardware
{
    int isHardware= (int)[[NSUserDefaults standardUserDefaults]  integerForKey:@"isHardware"];
    if(isHardware == 1)
    {
        UINavigationController* controllerWithHeart = [self.storyboard instantiateViewControllerWithIdentifier:@"MainViewControllerWithHeart"];
        [self presentViewController:controllerWithHeart animated:YES completion:nil];
...
- (void)isGetedHardwareVersion:(NSNotification *)aNotification
{
    [self toGetHardware];
    [[BackgroundManager sharedInstance] stopConnectTimer];
...
-(void)isSystemBleOn
{
    if([[LWBLEManager shared]toGetBLECentralStateOn])
    {
         [NSTimer scheduledTimerWithTimeInterval:1.0f  target:self  selector:@selector(isSystemBleOn) userInfo:nil  repeats:NO];    }
    else
    {
        [foundedDevices removeAllObjects];
        [self.tableView reloadData];
